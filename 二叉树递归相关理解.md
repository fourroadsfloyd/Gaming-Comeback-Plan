...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) 
    {
        ...
        int n = preorder.size();
        for(int i=0;i<n;i++)	//将中序遍历中的值与索引一一对应， key：值  value：index
        {
            umap[inorder[i]] = i;
        }    
                    ...
                    int ps, int pe, int is, int ie)
    {
        ...
        //传进的ps就对应一个父结点
        int proot = ps;
        int iroot = umap[preorder[proot]];	//找到中序遍历中该父结点的索引

        TreeNode* root = new TreeNode(preorder[proot]);

        int left_num = iroot - is;	//该父结点的左子树中存在多少个结点

        //找左子树，先序遍历范围：ps+1, ps+left_num. 中序遍历范围：is, iroot.
        root->left = build(preorder, inorder, ps+1, ps+left_num, is, iroot);
        //找右子树，先序遍历范围：ps+left_num+1, pe. 中序遍历范围：iroot+1, ie.
        root->right = build(preorder, inorder, ps+left_num+1, pe, iroot+1, ie);
        return root;
    }
private:
...
​	当ps > pe时，说明该子树在先序遍历序列中已经遍历完了

操作：

​	首先先将中序遍历中的值与其索引一一对应。

​	**按照先序遍历的顺序进行递归**，这里是指根据先序遍历中的某个结点，在中序遍历中找到其对应的左右子树。
...
​	首先建立起**子节点值和父结点**的映射关系，

​	根据传入的p,q，首先找到p的所有父结点，设为true，再去找q的父结点，当第一个true出现时，则是最近。
